...
class Kas extends REST_Controller {
	...
	function list_get(){
        ...
        $data = $this->mkas->list_bank_transaction($pSearch);
	...
	function submit_transaction_post(){
		...
		foreach ($varPost as $key => $value) {
			$keyNew = str_replace("MitraJaya_view_Report_Kas_MainGrid-FormBasicData-", '', $key);
			if ($value == "") {
				$value = null;
			}
		...
		if ($paramPost['BankTransactionID'] == '') {
            ...
            $proses = $this->mkas->insert_bank_transaction($paramPost);
        } else {
            $proses = $this->mkas->update_bank_transaction($paramPost);
        }
    ...
    function delete_transaction_delete(){
		...
		$query = $this->db->update("mj_kas", $data);
    ...
    public function generate_template_post(){
        ...
        try{
            ...
            */
            ...
            // set header 
            // Sheet 1
            $excel->getActiveSheet()->setTitle("Kas Template");
            $excel->getActiveSheet()->setCellValue('A1', 'TEMPLATE GENERATOR KAS');
            $excel->getActiveSheet()->setCellValue('A2', 'No');
            $excel->getActiveSheet()->setCellValue('B2', 'Date Transaction');
            $excel->getActiveSheet()->setCellValue('C2', 'Transaction No');
            $excel->getActiveSheet()->setCellValue('D2', 'Cost Element');
            $excel->getActiveSheet()->setCellValue('E2', 'Description');
            $excel->getActiveSheet()->setCellValue('F2', 'Transaction Type');
            $excel->getActiveSheet()->setCellValue('G2', 'Amount');
            $excel->getActiveSheet()->setCellValue('H2', 'Project');
            ...
            $excel->getActiveSheet()->setCellValue('D3', 'MSU02');
            $excel->getActiveSheet()->setCellValue('E3', 'Pembayaran');
            $excel->getActiveSheet()->setCellValue('F3', 'debit');
            $excel->getActiveSheet()->setCellValue('G3', '50000000');
            $excel->getActiveSheet()->setCellValue('H3', 'SALDO AWAL');
	...
	public function import_post(){
		...
		if ($this->upload->do_upload('kas_ImportFile')) {
            $file   = $this->upload->data();
            $reader = ReaderEntityFactory::createXLSXReader();
            $reader->open($_FILES['kas_ImportFile']['tmp_name']);
            ...
            foreach ($reader->getSheetIterator() as $sheet) {
                ...
                foreach ($sheet->getRowIterator() as $row) {
                    ...
                    if ($numRow >= 4) {
						...
						$Costelement = $this->mbank_transaction->CheckExistCostelement($cells[3]->getValue());
						...
						$ProjectID = $this->mbank_transaction->CheckExistProjectID($cells[7]->getValue());
						...
						$TransactionType = $cells[5]->getValue();
						$Amount 	= $cells[6]->getValue();
						$data['Costelement'] 		= $Costelement;
						$data['DateTransaction'] 	= @date_format($cells[1]->getValue(), 'Y-m-d');
						$data['NoVoucher'] 			= $cells[2]->getValue();
				...
				if(count($dataError) > 0){
					$this->db->truncate("mj_kas_tmp");
					$this->db->insert_batch("mj_kas_tmp", $dataError);
				}
                break;
            }
            //tutup spout reader
	...
	public function export_excel_post(){
        ...
        $dataList = $this->mkas->list_bank_transaction_excel($pSearch);
		...
		if(count($dataList['data'])){
			...
			$excel->getActiveSheet()->setTitle("Kas");
	...
	function list_import_failed_get(){
        ...
        // echo '<pre>'; print_r($pSearch); exit;
        $data = $this->mkas->list_import_failed($pSearch, $start, $limit, 'limit', $sortingField, $sortingDir);
        $this->response($data, 200);
	}
